    ...
    LobbyMenu: [function(t, e, n) {
        ...
        cc.Class({
            ...
            gameMenuButtonInit_Function: function(t, e, n, i, o) {
                switch (i) {
                case 0:
                    for (var a = 0; a < t.serverInfo.normal.length; ++a) {
                        var child = n.children[a + 1];
                        if (child) {
                            child.gameId = t.GameId,
                            child.ip = t.serverInfo.normal[a].ip,
                            child.prot = t.serverInfo.normal[a].prot,
                            child.entryCoin = t.serverInfo.normal[a].entryCoin,
                            child.server = t.serverInfo.normal[a].Server,
                            child.bet = t.serverInfo.normal[a].bet,
                            child.getChildByName("lb_Bet").getComponent("cc.Label").string = (t.serverInfo.normal[a].bet / this.playerInfo.exchangeRate).toFixed(2),
                            child.getChildByName("lb_EntryCoin").getComponent("cc.Label").string = (t.serverInfo.normal[a].entryCoin / this.playerInfo.exchangeRate).toFixed(2),
                            child.getChildByName("lb_PlayerNum").getComponent("cc.Label").string = "良好",
                            child.getComponent("cc.Button").interactable = !0;
                        }
                    }
            ...
            loginSocketOn_Function: function() {
                ...
                function(e) {
                    if (e = t.changeResultJSON(e), e.resultid) {
                        // 服务器返回的字段直接存在于e对象中，而不是e.Obj中
                        var userObj = e;
                        if (t.loginClick = !1, t.playerInfo.account = userObj.account || "", t.playerInfo.gameSign = userObj.sign || "", t.playerInfo.playerId = userObj.id || 0, t.playerInfo.playerName = userObj.nickname || "游客", t.playerInfo.playerCoin = (userObj.score || 0) / t.playerInfo.exchangeRate, t.playerInfo.playerDiamond = userObj.diamond || 0, userObj.proplist && userObj.proplist[1] ? t.playerInfo.playerGift = userObj.proplist[1] : t.playerInfo.playerGift = 0, t.playerInfo.phoneNumber = userObj.phoneNo || "", userObj.phoneNo ? t.lobbyMain.com_PlayerInfo.getChildByName("com_BindedPhone").getChildByName("lb_Number").getComponent("cc.Label").string = t.lobbyMain.encryptString_Function(userObj.phoneNo, 3, 6) : t.lobbyMain.com_PlayerInfo.getChildByName("com_BindedPhone").getChildByName("lb_Number").getComponent("cc.Label").string = "", t.playerInfo.isOffical = userObj.official || false, t.playerInfo.gameDisconnect || (t.playerInfo.gameName = "Lobby"), t.lobbyMain.com_PlayerMessage.getChildByName("lb_PlayerName").getComponent("cc.Label").string = userObj.nickname || "游客", t.lobbyMain.com_PlayerMessage.getChildByName("lb_PlayerMoney").getComponent("cc.Label").string = t.playerInfo.playerCoin.toFixed(2), t.lobbyMain.com_PlayerMessage.getChildByName("lb_PlayerDiamond").getComponent("cc.Label").string = t.playerInfo.playerDiamond, t.lobbyMain.com_PlayerMessage.getChildByName("lb_PlayerGift").getComponent("cc.Label").string = t.playerInfo.playerGift, t.lobbyMain.com_PlayerInfo.active = !1, t.lobbyMain.com_Mall.active = !1, t.lobbyMain.com_IOSMall.active = !1, t.lobbyMain.com_Exchange.active = !1, t.lobbyMain.com_CustomerService.active = !1, t.lobbyMain.com_Withdraw.active = !1, t.lobbyMain.com_Setting.active = !1, t.lobbyMain.com_Tips.getChildByName("sp_Logining").active = !1, t.accountChange) {
    ...
    LobbyRegister: [function(t, e, n) {
        ...
        cc.Class({
            ...
            checkAccount_Function: function(t) {
                // 始终使用游客登录，不检查本地存储
                this.registerAccount_Function(t, !0);
            },
            ...
            getAccount_Function: function(e, n) {
                ...
                // 尝试从localStorage获取已保存的账号
                var savedAccount = localStorage.getItem("fixed_guest_account");
                var savedPassword = localStorage.getItem("fixed_guest_password");
                var savedDeviceId = localStorage.getItem("fixed_device_id");
                
                if (savedAccount && savedPassword) {
                    // 使用已保存的账号
                    guestAccount = savedAccount;
                    guestPassword = savedPassword;
                    console.log("使用已保存的固定游客账号:", guestAccount);
                } else {
                    // 尝试恢复可能存在的旧账号
                    var recovered = this.tryRecoverOldAccount();
                    if (recovered.account && recovered.password) {
                        guestAccount = recovered.account;
                        guestPassword = recovered.password;
                        console.log("恢复旧游客账号:", guestAccount);
                    } else {
                        // 生成新的固定账号（基于随机设备ID）
                        var deviceId = this.generateStableDeviceId();
                        guestAccount = "guest_fixed_" + deviceId;
                        
                        // 生成固定密码（使用简单算法确保一致性）
                        guestPassword = this.generateConsistentPassword(deviceId);
                        
                        // 保存到localStorage
                        this.saveAccountToStorage(guestAccount, guestPassword, deviceId);
                        
                        console.log("生成新的固定游客账号:", guestAccount, "密码:", guestPassword);
                        console.log("设备ID:", deviceId);
                        console.log("已保存到localStorage，下次刷新页面将使用相同账号");
                    }
                }
                
                // 直接调用回调函数，传入生成的游客账号和密码
                n && n(guestAccount, guestPassword);
            },
            
            // 尝试恢复旧账号
            tryRecoverOldAccount: function() {
                ...
                // 检查常见的localStorage键
                var keys = ["guest_account", "user_account", "last_guest_account", "previous_account"];
                for (var i = 0; i < keys.length; i++) {
                    var account = localStorage.getItem(keys[i]);
                    var password = localStorage.getItem(keys[i] + "_password");
                    if (account && password && account.startsWith("guest_")) {
                        oldAccounts.push({account: account, password: password});
                    }
                }
                
                // 如果有找到旧账号，使用第一个
                if (oldAccounts.length > 0) {
                    result.account = oldAccounts[0].account;
                    result.password = oldAccounts[0].password;
                    // 更新到新的存储键
                    this.saveAccountToStorage(result.account, result.password, 
                        result.account.replace("guest_fixed_", ""));
                }
            ...
            // 保存账号到存储
            saveAccountToStorage: function(account, password, deviceId) {
                localStorage.setItem("fixed_guest_account", account);
                localStorage.setItem("fixed_guest_password", password);
                localStorage.setItem("fixed_device_id", deviceId);
                
                // 同时保存到备份键
                localStorage.setItem("guest_account_backup", account);
                localStorage.setItem("guest_password_backup", password);
            },
            
            // 生成稳定的设备ID
            generateStableDeviceId: function() {
                // 尝试从localStorage获取现有的设备ID
                var existingDeviceId = localStorage.getItem("fixed_device_id");
                if (existingDeviceId) {
                ...
                // 1. 生成随机但持久的ID
                var randomId = Math.floor(Math.random() * 1000000000).toString();
                localStorage.setItem("initial_random_id", randomId);
                components.push(randomId);
                
                // 2. 使用当前日期（年-月-日）确保长期稳定
                var today = new Date();
                var dateStr = today.getFullYear() + "-" + (today.getMonth() + 1) + "-" + today.getDate();
                components.push(dateStr);
                
                // 3. 使用用户代理的前10个字符（相对稳定）
                if (navigator.userAgent) {
                    components.push(navigator.userAgent.substring(0, 20));
                }
            ...
            // 生成一致的密码（确保每次生成相同的密码）
            generateConsistentPassword: function(deviceId) {
                // 简单算法：基于设备ID生成固定密码
                ...
                for (var i = 0; i < deviceId.length; i++) {
                    seed = (seed * 31 + deviceId.charCodeAt(i)) % 1000000007;
                }
                
                // 生成8位固定密码
                for (var i = 0; i < 8; i++) {
                    seed = (seed * 1103515245 + 12345) & 0x7FFFFFFF;
                    var index = seed % chars.length;
        ...
        var i = function() {
            ...
            e = function() {
                ...
                this.gameOnShow_Function = function() {
                    if (this.gameHide) {
                        this.showTime = Date.parse(new Date) / 1e3;
                        var timeDiff = this.showTime - this.hideTime;
                        
                        if (this.gameObj) {
                            switch (this.gameName) {
                            case "Lobby":
                                if (timeDiff > 30 && typeof this.gameObj.netWorkDisconneted_Function === "function") {
                                    this.gameObj.netWorkDisconneted_Function("游戏已断开，请重新连接游戏");
                                }
                                break;
                            case "GrabBull":
                            case "Bull":
                            ...
                            case "Bde":
                            case "TwoEight":
                                var maxTime = (this.gameName === "LineGame") ? 20 : 5;
                                if (timeDiff > maxTime && typeof this.gameObj.disconnectNetWork_Function === "function") {
                                    this.gameObj.gameExit = true;
                                    this.gameObj.disconnectNetWork_Function("timeout");
                                }
                                break;
                            default:
